...
3. `restoreBody` restores only tokens in the allowlist:
   - JSON bodies are decoded, restored inside string values, and re-encoded with HTML escaping disabled so the angle brackets survive. This keeps quotes, backslashes, and newlines in the secret valid.
...
1. On the stream-init call (`ChunkIndex == -1`) the plugin resets any leftover reassembly buffer for this stream.
2. For each data chunk it prepends raw carry from the previous host chunk and uses the same vault-verified, 1,024-token per-request allowlist.
...
4. After raw chunk carry, protocol adapters reassemble possible token prefixes across semantic visible-text events for OpenAI Chat Completions, OpenAI Responses, Claude, and Gemini. Pending text is isolated by protocol and choice/item/block/candidate channel, and only a possible trailing token prefix is retained. Gemini streams may contain SSE-framed JSON events or complete bare JSON responses even while the Content-Type remains `text/event-stream`.
5. If a whole chunk is withheld by the raw carry layer, the plugin returns `DropChunk` so the host does not deliver a still-tokenized fragment.
6. Raw withheld data is capped at 1 MiB per stream. An oversized unfinished event or over-limit semantic prefix is emitted unchanged rather than retained or restored unsafely.
7. If restoration turns an entire stream chunk into zero bytes, the plugin returns `DropChunk` so the original placeholder is not delivered.
...
- **Streaming edge cases.** The host exposes no end-of-stream flush callback and no stable per-stream identity. Bytes withheld to reassemble a token split across the final chunk can be lost, and concurrent streams with byte-identical request bodies share a reassembly buffer. The plugin retains SHA-256 request-body keys rather than using a collision-prone faster hash; eliminating that per-chunk request-body hash and providing full stream isolation requires a host-provided stream ID.
- **Streaming semantic scope.** Cross-event semantic reassembly covers visible assistant text only. Reasoning and tool-argument values retain single-event restoration behavior, so a token in those values is restored only when it occurs complete within one event; a token split across events can remain unrestored.
