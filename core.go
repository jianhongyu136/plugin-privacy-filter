...
func handleRequestIntercept(request []byte) (out []byte, err error) {
	...
	// convert internal failures into an explicit Reject response here instead.
	defer func() {
		if r := recover(); r != nil {
			out, err = okEnvelope(pluginapi.RequestInterceptResponse{
				Reject:       true,
				RejectReason: "privacy-filter panic while redacting request",
			})
		}
	}()
	...
	if e := json.Unmarshal(request, &req); e != nil {
		return okEnvelope(pluginapi.RequestInterceptResponse{
			Reject:       true,
			RejectReason: "privacy-filter could not parse request: " + e.Error(),
		})
	}
	st := activeSnapshot()
	if st == nil {
		...
		return okEnvelope(pluginapi.RequestInterceptResponse{
			Reject:       true,
			RejectReason: "privacy-filter not configured",
		})
	}
	...
	case dispositionReject:
		...
		return okEnvelope(pluginapi.RequestInterceptResponse{
			Reject:       true,
			RejectReason: "privacy-filter does not support source format: " + req.SourceFormat,
		})
	}
	...
	if scanErr != nil {
		...
		}).Warn("privacy-filter rejected unscannable request content")
		return okEnvelope(pluginapi.RequestInterceptResponse{
			Reject:       true,
			RejectReason: "privacy-filter could not scan request content at " + scanErr.Path + ": " + scanErr.Detail,
		})
	}
	if !handled {
		...
		return okEnvelope(pluginapi.RequestInterceptResponse{
			Reject:       true,
			RejectReason: "privacy-filter could not parse " + req.SourceFormat + " request body",
		})
	}
	...
	if st.mode == modeBlock && len(result.Matches) > 0 {
		return okEnvelope(pluginapi.RequestInterceptResponse{
			Reject:       true,
			RejectReason: blockRejectReason(result.Matches),
		})
	}
